    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
          <span style={{ background: '#FEF3C7', color: '#D97706', borderRadius: 20, padding: '4px 12px', fontSize: 12, fontWeight: 600 }}>
            ⏱ Prep: {pastel.tiempo_preparacion} min
          </span>
          {pastel.tiempo_coccion > 0 && (
            <span style={{ background: '#FEE2E2', color: '#DC2626', borderRadius: 20, padding: '4px 12px', fontSize: 12, fontWeight: 600 }}>
      ...
      {expandido && (
        <div style={{ padding: '0 24px 24px', borderTop: '1px solid #f3f4f6' }}>
          <div style={{ marginBottom: 20 }}>
            <p style={{ margin: '16px 0 10px', fontSize: 12, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: 0.5 }}>🛒 Ingredientes</p>
          ...
          <div>
            <p style={{ margin: '0 0 10px', fontSize: 12, fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: 0.5 }}>👩‍🍳 Preparación</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {pastel.preparacion.map((paso, i) => (
                <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', background: '#f8fafc', borderRadius: 10, padding: '10px 14px' }}>
                  <span style={{ background: '#E8821A', color: 'white', borderRadius: '50%', width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>{i + 1}</span>
...
function AperitivoCard({ aperitivo }: { aperitivo: Aperitivo }) {
  ...
  return (
    <div style={{ background: 'white', borderRadius: 16, border: '1px solid #e5e7eb', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,.04)' }}>
      <div style={{ padding: '16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
          <span style={{ fontSize: 28 }}>{aperitivo.emoji}</span>
          <h4 style={{ margin: 0, fontFamily: "'Fredoka',sans-serif", fontSize: 16, fontWeight: 600, color: '#1f2937' }}>{aperitivo.nombre}</h4>
        </div>
        <p style={{ margin: '0 0 10px', fontSize: 13, color: '#6b7280', lineHeight: 1.4 }}>{aperitivo.descripcion}</p>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
          <span style={{ background: '#FEF3C7', color: '#D97706', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 600 }}>⏱ {aperitivo.tiempo_preparacion} min</span>
          <span style={{ background: '#F0FDFA', color: '#0d9488', borderRadius: 20, padding: '3px 10px', fontSize: 11, fontWeight: 600 }}>{aperitivo.para_edad}</span>
        </div>
        <button
          onClick={() => setExpandido(!expandido)}
          style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, fontSize: 12, fontWeight: 600, color: '#E8821A', fontFamily: "'Outfit',sans-serif", display: 'flex', alignItems: 'center', gap: 4 }}
        >
...
function ResultadosIdeas({ resultado }: { resultado: ResultadoCumpleanos }) {
  return (
    <div>
      <div style={{ marginBottom: 40 }}>
        <h2 style={{ fontFamily: "'Fredoka',sans-serif", fontSize: 24, fontWeight: 600, color: '#1f2937', margin: '0 0 6px' }}>
          🎂 Opciones de pastel
        </h2>
        <p style={{ color: '#6b7280', fontSize: 14, margin: '0 0 20px' }}>3 recetas de pastel saludable para celebrar</p>
      ...
      <div>
        ...
        <p style={{ color: '#6b7280', fontSize: 14, margin: '0 0 20px' }}>15 ideas de bocadillos nutritivos para los invitados</p>
...
export default function CumpleanosGenerador({
  ...
  pais: string
  hijoId: string | null
  resultadoGuardado: ResultadoCumpleanos | null
}) {
  ...
  async function generarIdeas() {
    ...
    try {
      const res = await fetch('/api/cumpleanos', {
        ...
        body: JSON.stringify({ edadCumple, nombreBebe, numInvitados, pais, hijoId }),
      })
  ...
  return (
    <div>
      ...
      {generando && (
        <div style={{
          ...
          <div style={{
            ...
            <h3 style={{ margin: '0 0 8px', fontFamily: "'Fredoka',sans-serif", fontSize: 22, color: '#1f2937' }}>Preparando ideas</h3>
          ...
          <div style={{
            ...
            <div>
              <p style={{ margin: 0, fontSize: 14, color: '#15803d', fontWeight: 600 }}>
                Ideas generadas este mes
              </p>
              <p style={{ margin: '2px 0 0', fontSize: 12, color: '#16a34a' }}>
                Podrás generar nuevas ideas el {proximoMes()}
              </p>
            </div>
          </div>
        ) : (
      ...
      {/* Error */}
      {error && (
        <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 14, padding: '14px 18px', marginBottom: 24 }}>
          <p style={{ margin: 0, color: '#DC2626', fontSize: 14 }}>⚠️ {error}</p>
        </div>
      )}

      {/* Results */}
      {resultado && <ResultadosIdeas resultado={resultado} />}
    </div>
  )
}
